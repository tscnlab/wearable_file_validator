...
Additional non-tabular header lines above the column names are discouraged. If such lines are unavoidable, their number and content format must be consistent across files from the same device model and software version, and they must be documented in the metadata descriptor.
